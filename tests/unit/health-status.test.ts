...
describe('resolveHealthStatus', () => {
  it('returns ok when database is ok or skipped', () => {
    ...
    expect(resolveHealthStatus('ok', 'error')).toBe('ok');
  });

  it('returns degraded when database errors', () => {
    expect(resolveHealthStatus('error', 'ok')).toBe('degraded');
    expect(resolveHealthStatus('error', 'error')).toBe('degraded');
    expect(resolveHealthStatus('error', 'skipped')).toBe('degraded');
  });
});
